...
public class rmg_api_02_AddProjectWithOnGoingStatusTest extends ApiBaseClass {
	@Test
	public void addProjectWithCompletedStatusTest() throws Throwable {
		
		String status="On Going";
		String projectName = "deepak_pro-2"+JavaUtility.getRandomData();
		Project_PojoClass pojo=new Project_PojoClass("arijit", projectName, status, 10);
		Response response = given()
			.contentType(ContentType.JSON)
			.body(pojo)
		.when()
			.post(IConstants.URI+IEndpoints.AddProjectWithOnGoingStatusTest_Endpoints);
		response.then()
